...
async function bootstrap(): Promise<void> {
  ...
  await withRequestDatabase(async (database) => {
    let [organization] = await database.select().from(organizations).where(eq(organizations.slug, environment.SEED_ORG_SLUG)).limit(1);
    if (!organization) {
      [organization] = await database.insert(organizations).values({ slug: environment.SEED_ORG_SLUG, name: environment.SEED_ORG_NAME }).returning();
    }

    let [owner] = await database.select().from(users).where(eq(users.email, environment.SEED_ADMIN_EMAIL)).limit(1);
    if (!owner) {
      [owner] = await database.insert(users).values({
    ...
    await database.insert(memberships).values({ organizationId: organization.id, userId: owner.id, role: "OWNER" }).onConflictDoNothing();
    await database.insert(organizationSequences).values({ organizationId: organization.id, nextWorkOrderNumber: 1001 }).onConflictDoNothing();
