...
import { getRouteSessionIdentity, getSessionIdentity, type SessionIdentity } from "./session";
...
export async function resolveTenantMembership(database: FlowDeskDatabase, orgSlug: string, session: SessionIdentity): Promise<TenantContext | null> {
...
export async function resolveTenantContext(database: FlowDeskDatabase, orgSlug: string): Promise<TenantContext | null> {
  ...
  const session = await getSessionIdentity(database);
  return session ? resolveTenantMembership(database, orgSlug, session) : null;
}

export async function requireTenantPageContext(database: FlowDeskDatabase, orgSlug: string, returnTo = `/app/${orgSlug}/dashboard`): Promise<TenantContext> {
  ...
  const context = await resolveTenantMembership(database, orgSlug, session);
  if (!context) notFound();
  return context;
}

export async function requireTenantApiContext(database: FlowDeskDatabase, orgSlug: string): Promise<TenantContext> {
  ...
  const session = await getRouteSessionIdentity(database);
...
export async function getDefaultTenantSlug(database: FlowDeskDatabase): Promise<string | null> {
